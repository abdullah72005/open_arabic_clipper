...
_LIFECYCLE_AFTER = (*_LIFECYCLE_BEFORE, "AUDIO_EXTRACTION", "TRANSCRIPTION", "TRANSCRIPT_NORMALIZATION", "AUDIO_ANALYSIS", "READY_FOR_ANALYSIS")
_JOB_BEFORE = ("INGEST", "PROBE")
_JOB_AFTER = (*_JOB_BEFORE, "TRANSCRIPTION")
_PIPELINE_BEFORE = ("PROBE", "READY_FOR_TRANSCRIPTION")
_PIPELINE_AFTER = (*_PIPELINE_BEFORE, "AUDIO_EXTRACTION", "TRANSCRIPTION", "TRANSCRIPT_NORMALIZATION", "AUDIO_ANALYSIS", "READY_FOR_ANALYSIS")
...
def upgrade() -> None:
    ...
    op.create_index("ix_audio_artifacts_source_content_hash", "audio_artifacts", ["source_content_hash"])
    ...
    op.create_index("ix_source_quality_assessments_source_video_id", "source_quality_assessments", ["source_video_id"])


def downgrade() -> None:
    op.drop_index("ix_source_quality_assessments_source_video_id", table_name="source_quality_assessments")
    ...
    with op.batch_alter_table("pipeline_runs") as batch:
        batch.alter_column("stage", existing_type=_enum("pipeline_stage", _PIPELINE_AFTER), type_=_enum("pipeline_stage", _PIPELINE_BEFORE), existing_nullable=False)
    with op.batch_alter_table("processing_jobs") as batch:
        batch.alter_column("kind", existing_type=_enum("job_kind", _JOB_AFTER), type_=_enum("job_kind", _JOB_BEFORE), existing_nullable=False)
    with op.batch_alter_table("source_videos") as batch:
        batch.alter_column("lifecycle_state", existing_type=_enum("source_lifecycle_state", _LIFECYCLE_AFTER), type_=_enum("source_lifecycle_state", _LIFECYCLE_BEFORE), existing_nullable=False)
