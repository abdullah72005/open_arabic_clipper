"""Explicit local (Qwen/Ollama) Stage 4.2 governance adapter.
...
claim external verification or a platform outcome.
"""
...
from app.transformation.governance.policy import GOVERNOR_SCHEMA_VERSION
...
class LocalGovernanceProvider:
    ...
    def __init__(
        ...
        *,
        ...
        self._digest = hashlib.sha256(
            f"ollama-transformation-governance:{model}".encode()
        ).hexdigest()
    ...
    def govern(self, request: GovernanceRequest, tier: str = "ROUTINE") -> ProviderGovernanceResult:
        instruction = GOVERNANCE_SYSTEM_INSTRUCTION
        ...
        body = {
            ...
            "messages": [
                ...
                {"role": "user", "content": json.dumps(request.to_payload(), ensure_ascii=False)},
            ],
        }
    ...
    def _call(self, body: dict[str, object]) -> dict[str, object]:
        ...
        try:
            ...
            raise GovernanceProviderError(
                ProviderErrorCategory.MALFORMED_OUTPUT.value, "local governance output invalid"
            ) from error
        except (KeyError, IndexError, TypeError, UnicodeDecodeError, json.JSONDecodeError, OSError):
            raise GovernanceProviderError(
                ProviderErrorCategory.PROVIDER_ERROR.value, "local governance request failed"
            ) from None
