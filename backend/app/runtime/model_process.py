...
class ProcessOutcome:
    ...
    ``child_peak_rss_bytes`` is the peak measured inside the child after the
    model work finished, or None when the child exited abnormally (timeout,
    cancellation, or death without an envelope) and no trustworthy peak exists.
    None is reported as UNKNOWN; a fabricated zero is never treated as a peak.
    """
...
def _child_entrypoint(
    ...
    Peak RSS is read after the target finishes so the reported peak reflects the
    loaded model at work, never the child's pre-load RSS.
    """
...
class ModelProcessRunner:
    ...
    def run(
        ...
        *,
        ...
        When ``cancel_event`` is set, the parent terminates the child and reaps
        it immediately. The lease-loss path uses this so an ownership loss never
        leaves a heavy model resident in a spawned child.
        """
        ...
        while envelope is None:
            if cancel_event is not None and cancel_event.is_set():
                ...
                return ProcessOutcome(
                    False,
                    None,
                    "child cancelled after heavy-model lease loss",
                    process.pid,
                    process.exitcode,
                    None,
