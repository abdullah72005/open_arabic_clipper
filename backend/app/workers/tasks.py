...
@celery_app.task(  # type: ignore[untyped-decorator]
    bind=True, autoretry_for=(), name="clipfactory.run_transformation_planning"
)
def run_transformation_planning(
    self: Task,
    plan_set_id: str,
    job_id: str | None = None,
    force: bool = False,
) -> dict[str, str | bool | None]:
    """Run one explicit candidate-scoped Stage 4.1 transformation planning run.
    ...
    parsed_plan_set = _UUID(plan_set_id)
    parsed_job = _UUID(job_id) if job_id else None
    session = create_session_factory()()
    try:
        settings = get_settings()
